...
class EventPhaseDetail(BaseModel):
    ...
    decision_text: str = Field(
        default="", description="决策摘要（由 DecisionTextExtractor 后填）"
    )
...
class CharacterAppearance(BaseModel):
    ...
    distinctive_features: Optional[list[str]] = Field(
        default=None, description="标志性特征"
    )
...
class Character(BaseModel):
    ...
    aliases: Optional[list[str]] = Field(
        default_factory=list, description="其他称呼/别名"
    )
...
class Precondition(BaseModel):
    ...
    from_value: Optional[str] = Field(
        default=None,
        description="事件开始前的归属（unnecessary 实体置 null）",
        alias="from",
    )
...
class Effect(BaseModel):
    ...
    from_value: Optional[str] = Field(
        default=None,
        description="变化前的归属（unnecessary 实体置 null）",
        alias="from",
    )
    to: Optional[str] = Field(
        default=None, description="变化后的归属（unnecessary 实体置 null）"
    )
    granularity: Literal["named", "functional"]
...
class EventNecessity(BaseModel):
    ...
    def _drop_incomplete_reasoning(cls, v: object) -> object:
        ...
        return [
            item for item in v if isinstance(item, dict) and required.issubset(item)
        ]
